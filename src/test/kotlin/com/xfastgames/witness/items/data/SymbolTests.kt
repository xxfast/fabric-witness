...
/**
 * Hexagons as a [Symbol] held separately from a node's role and an edge's traversal state
...
class SymbolTests {
    ...
    private val dotted = Node(1f, 0f, symbol = Symbol.HEXAGON)

    @Test
    fun `A node's symbol survives a round trip`() {
        val tag: NbtCompound = NbtCompound().apply { putNode(dotted) }
    ...
    fun `A node carries a role and a symbol at once`() {
        val start = Node(0f, 0f, Modifier.START, Symbol.HEXAGON)
        val tag: NbtCompound = NbtCompound().apply { putNode(start) }

        val actual: Node = tag.getNode()
        assertThat(actual.modifier).isEqualTo(Modifier.START)
        assertThat(actual.symbol).isEqualTo(Symbol.HEXAGON)
    }
    ...
    fun `An edge's symbol survives a round trip`() {
        val graph: MutableValueGraph<Node, Edge> = ValueGraphBuilder.undirected()
            .build<Node, Edge>()
            .apply { putEdgeValue(plain, dotted, Edge(Modifier.NORMAL, Symbol.HEXAGON)) }
        val tag: NbtCompound = NbtCompound().apply { putValueGraph(KEY_GRAPH, graph) }
    ...
    fun `A broken edge keeps its state and its symbol independently`() {
        val graph: MutableValueGraph<Node, Edge> = ValueGraphBuilder.undirected()
            .build<Node, Edge>()
            .apply { putEdgeValue(plain, dotted, Edge(Modifier.BREAK, Symbol.HEXAGON)) }
        val tag: NbtCompound = NbtCompound().apply { putValueGraph(KEY_GRAPH, graph) }
    ...
    fun `Growing a grid carries a node's symbol across`() {
        val source: Panel.Grid = gridOf(Node(0.5f, 0.5f, Modifier.START, Symbol.HEXAGON))

        val expanded: Panel.Grid = source.expandTo(3, 3)

        val carried: Node = expanded.graph.nodes().single { it.symbol == Symbol.HEXAGON }
    ...
    inner class Legacy {
        ...
        fun `A node whose modifier was a hexagon becomes a roleless node carrying one`() {
            val tag: NbtCompound = legacyNode(x = 2f, y = 3f, modifier = Modifier.DOT)

            assertThat(tag.getNode()).isEqualTo(Node(2f, 3f, Modifier.NONE, Symbol.HEXAGON))
        }

        @Test
        fun `An edge whose value was a hexagon becomes a traversable edge carrying one`() {
            val tag: NbtCompound = legacyGraph(Modifier.DOT)

            val actual: Edge = requireNotNull(
                tag.getValueGraph(KEY_GRAPH).edgeValue(plain, Node(1f, 0f)).orElse(null)
            )
            assertThat(actual).isEqualTo(Edge(Modifier.NORMAL, Symbol.HEXAGON))
        }
        ...
        fun `A graph with no edge symbol array keeps every edge`() {
            ...
            val tag: NbtCompound = legacyGraph(Modifier.NORMAL)
        ...
        fun `An unknown modifier ordinal reads as none instead of throwing`() {
            ...
            val tag: NbtCompound = NbtCompound().apply {
                putFloat("x", 0f)
                putFloat("y", 0f)
                putInt("modifier", 99)
            }

            assertThat(tag.getNode()).isEqualTo(Node(0f, 0f, Modifier.NONE, Symbol.NONE))
        }
    }
    ...
    inner class Authoring {
        ...
        fun `Toggling a node on and off leaves the panel as it was`() {
            ...
            assertThat(marked.graph.nodes().single { it.symbol == Symbol.HEXAGON }.modifier)
                .isEqualTo(Modifier.START)
            assertThat(cleared.graph.nodes().none { it.symbol == Symbol.HEXAGON }).isTrue()
        }
        ...
        fun `Marking a node keeps its role and its edges`() {
            ...
            assertThat(start.symbol).isEqualTo(Symbol.HEXAGON)
        ...
        fun `Marking an edge keeps it traversable`() {
            ...
            assertThat(marked.graph.edgeValue(a, b).orElse(null))
                .isEqualTo(Edge(Modifier.NORMAL, Symbol.HEXAGON))
        }
        ...
        fun `A node wins over the edges that meet it`() {
            ...
            val start: Node = marked.graph.nodes().single { it.symbol == Symbol.HEXAGON }
            assertThat(marked.graph.edgeValue(start, b).orElse(null)?.symbol).isEqualTo(Symbol.NONE)
            assertThat(marked.graph.nodes().count { it.symbol == Symbol.HEXAGON }).isEqualTo(1)
        }
    ...
    private fun legacyNode(x: Float, y: Float, modifier: Modifier): NbtCompound =
        NbtCompound().apply {
            putFloat("x", x)
            putFloat("y", y)
    ...
    private fun legacyGraph(edge: Modifier): NbtCompound = NbtCompound().apply {
        put(KEY_GRAPH, NbtCompound().apply {
            put("nodes", NbtList().apply {
